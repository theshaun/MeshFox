...
if ! systemctl is-enabled meshfox-runonce &>/dev/null; then # if not the first boot
  ...
  export LANG=C.UTF-8
  dialog --title "MeshFox run once utility" --yesno "\
...
* Add changes to user meshfox's .bashrc\n\
\n\
Finally, the MeshFox will reboot.\n\
\n\
...
export TERM=xterm-256color
export LANG=C.UTF-8"
if ! grep -Fxq "$lines" /home/meshfox/.bashrc; then # Check if the lines are already in .bashrc
  log_message "Adding TERM, LANG and NCURSES_NO_UTF8_ACS to .bashrc..."
  echo "$lines" >> /home/meshfox/.bashrc
else
...
if ! grep -Fxq "alias smc='sudo meshfox-config'" /home/meshfox/.bashrc; then # Check if the lines are already in .bash_aliases
  echo "alias smc='sudo meshfox-config'" >> /home/meshfox/.bashrc
  log_message "Added \`alias smc='sudo meshfox-config'\` to .bashrc"
else
  log_message "\`alias smc='sudo meshfox-config'\` already present in .bashrc, skipping..."
fi
...
systemctl disable meshfox-runonce
